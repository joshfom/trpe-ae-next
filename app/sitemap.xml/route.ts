...
export async function GET(): Promise<Response> {
  try {
    // Check if database is available (important for Cloudflare deployments)
    if (!db) {
    ...
    // Return a comprehensive basic sitemap with static URLs if database fails
    ...
    return new Response(basicSitemap, {
      ...
      status: 200, // Return 200 even on error to provide a valid sitemap
    });
  }
}
