...
import { useUpdateCommunityV2 } from '@/features/admin/community/api/use-update-community-v2';
import { useGetCommunity } from '@/features/admin/community/api/use-get-community';
...
export default function EditCommunityPage({ params }: EditCommunityPageProps) {
    ...
    const { data: community, isLoading, error } = useGetCommunity(communityId);
    const mutation = useUpdateCommunityV2(communityId);
    ...
    useEffect(() => {
        if (community) {
            ...
            setHasDefaultImage(!!community.image);
        }
    }, [community, defaultValues, form]);
    ...
    if (error || !community) {
        return (
            <div className="flex justify-center items-center py-8">
                <div className="text-red-500">Error loading community: {error?.message || 'Community not found'}</div>
            </div>
        );
    }

    const displayName = community.name || 'Unnamed Community';
        ...
        <div className="container mx-auto py-8 px-4 max-w-4xl">
            ...
            <div className="flex items-center gap-4 mb-8">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-muted-foreground">Editing: {displayName}</p>
                </div>
            </div>
                        ...
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                                                ...
                                                {(field.value || hasDefaultImage) && (
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm text-muted-foreground">
                                                            {field.value ? 'Image uploaded successfully' : 'Using existing image'}
                                                        </span>
                                                        <Button
                                                            type="button"
                                                            variant="outline"
                            ...
                            <div className="flex gap-4 pt-6">
                                ...
                                <Button 
                                    ...
                                    className="flex-1"
                                >
                                    {mutation.isPending ? 'Saving...' : 'Save Community'}
                                </Button>
                            </div>
                        </form>
                    </Form>
